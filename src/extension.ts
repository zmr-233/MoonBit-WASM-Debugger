...
async function handleMoonRunTask(context: vscode.ExtensionContext, task: vscode.Task, workspaceFolder: string) {
  ...
  const nonOptionArgs = args.filter((arg: string) => !arg.startsWith('-') && !arg.startsWith('--'));
  const mainFileArgs = nonOptionArgs.filter((arg: string) => arg !== 'moon' && arg !== 'run');
  ...
  if (!args.includes('-g') && !args.includes('--debug')) {
    ...
    const newCommand = `moon clean && ${Top.command} --debug`;
